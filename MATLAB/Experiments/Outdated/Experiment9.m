...
stdThrustOutputUncertaintyFactor = 0.02;
...
%MCthrustOutputDeviation = (meanThrustOutputFactor-thrustOutputUncertaintyFactor : (2*thrustOutputUncertaintyFactor)/(MCsampleNum-1) : meanThrustOutputFactor+thrustOutputUncertaintyFactor)';


meanThrustDirectionErrorRollDeg = 0;
stdThrustDirectionErrorRollDeg = 0.05;
...
stdThrustDirectionErrorPitchDeg = 0.05;
...
stdThrustDirectionErrorYawDeg = 0.05;
...
[ QmatECItoLVLH_targetEnd ] = ECIToLVLH( rECIManouverEnd_target, vECIManouverEnd_target );
...
for MCIter = 1:MCsampleNum
    ...
    for trajectoryIter = 1:N_Step+1
        thisECITargetPos = [ targetEph(trajectoryIter, 2); targetEph(trajectoryIter, 3); targetEph(trajectoryIter, 4) ]./10^3;
        thisECITargetVel = [ targetEph(trajectoryIter, 5); targetEph(trajectoryIter, 6); targetEph(trajectoryIter, 7) ]./10^3;
...
hold on
grid on
histogram(absDeviationEndPosHPOP_1,'Normalization','probability','BinWidth',1)
xlabel('Distance [km]')
ylabel('Probability')
...
%% Initial Full Orbit Determination
...
% B's ideal position at experiment start
chaserY0 = [ r0ECI_chaser', v0ECI_chaser' ].*10^3;
[chaserEph] = ephemeris_v3(chaserY0, N_Step, Step);

%%
figure(13)
hold on
grid on
title('ECI Initial Trajectories')
%axis equal
xlabel('X [km]')
ylabel('Y [km]')
zlabel('Z [km]')
%[ sx, sy, sz ] = sphere;
%surf( sx*rEarth, sy*rEarth, sz*rEarth, 'FaceAlpha', 0.05, 'EdgeAlpha', 0.05 );
plot3(targetEph(:,2)./10^3, targetEph(:,3)./10^3, targetEph(:,4)./10^3, 'r')
plot3(chaserEph(:,2)./10^3, chaserEph(:,3)./10^3, chaserEph(:,4)./10^3, 'b')
%plot3( rECIManouverEnd_target(1), rECIManouverEnd_target(2), rECIManouverEnd_target(3), '*k' )
text( rECIManouverEnd_target(1), rECIManouverEnd_target(2), rECIManouverEnd_target(3), 'Chaser Ideal Maneuver End' )
hold off
