function outdata = jatmos70(r_Sun, indata)
...
%  indata(13) = Greenwich apparent sidereal time [rad]
% 
% Outputs:
...
[sda, sha, dd, dy] = jtme(r_Sun, mn, ida, iyr, ihr, min, xlng, indata(13));
...
function [sda, sha, dd, dy] = jtme (r_Sun, mn, ida, iyr, ihr, min, xlng, gst)

% compute day number of the year
dd = finddays (iyr, mn, ida, ihr, min, 0);

% compute fraction of tropical year
dy = dd / 365.2422;

% Sun right ascension, declination
ras  = atan2(r_Sun(2), r_Sun(1));
sda  = atan2(r_Sun(3), sqrt(r_Sun(1)^2+r_Sun(2)^2 ));

% compute right ascension of point of interest
rap = mod(gst + xlng, 2 * pi);

% compute hour angle
sha = mod(rap - ras, 2 * pi);
