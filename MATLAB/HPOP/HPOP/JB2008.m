...
%
%***********************************************************************
function [TEMP,RHO] = JB2008(MJD,r_Sun,r_SAT)

global SOLdata DTCdata
...
ra_SAT  = atan2(r_SAT(2), r_SAT(1));
dec_SAT = atan2(r_SAT(3), sqrt(r_SAT(1)^2+r_SAT(2)^2));
SAT(1) = mod(ra_SAT, 2*pi);
SAT(2) = dec_SAT;
[~, ~, height] = Geodetic(r_SAT);
SAT(3) = height/1000;
