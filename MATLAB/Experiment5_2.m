...
%%%%%%%  Experiment 5 Thrust effect d = 20s and all other effects


AuxParam.thrustDuration = 7;
AuxParam.Mjd_UTC = Mjd_UTC;
AuxParam.n       = 40;
AuxParam.m       = 40;
AuxParam.sun     = 1;
...
MC_5_HPOP_PosEnd = zeros( MCsampleNum, 3);
MC_5_HPOP_PosStart = zeros( MCsampleNum, 3);
MC_5_HPOP_PosEndMean = zeros(MCsampleNum, 3);
MC_5_HPOP_PosStartMean = zeros(MCsampleNum, 3);

MC_5_HPOP_VelEnd = zeros(MCsampleNum, 3);
MC_5_HPOP_VelStart = zeros(MCsampleNum, 3);
MC_5_HPOP_VelEndMean = zeros(MCsampleNum, 3);
MC_5_HPOP_VelStartMean = zeros(MCsampleNum, 3);

MC_5_HPOP_ECI_X_Trajectories = zeros(MCsampleNum, N_Step+1);
MC_5_HPOP_ECI_Y_Trajectories = zeros(MCsampleNum, N_Step+1);
MC_5_HPOP_ECI_Z_Trajectories = zeros(MCsampleNum, N_Step+1);
...
MC_5_HPOP_PosEndMean( 1, : ) = MC_5_HPOP_PosEnd( 1, : );
MC_5_HPOP_VelEndMean( 1, : ) = MC_5_HPOP_VelEnd( 1, : );

MC_5_HPOP_ECI_X_Trajectories(1, :) = Eph(:, 2)'./10^3;
MC_5_HPOP_ECI_Y_Trajectories(1, :) = Eph(:, 3)'./10^3;
MC_5_HPOP_ECI_Z_Trajectories(1, :) = Eph(:, 4)'./10^3;
...
for experimentIndex = 2 : MCsampleNum
    ...
    MC_5_HPOP_PosEndMean( experimentIndex, : ) = mean(MC_5_HPOP_PosEnd( 1 : experimentIndex, : ));
    MC_5_HPOP_VelEndMean( experimentIndex, : ) = mean(MC_5_HPOP_VelEnd( 1 : experimentIndex, : ));
    
    MC_5_HPOP_ECI_X_Trajectories(experimentIndex, :) = Eph(:, 2)'./10^3;
    MC_5_HPOP_ECI_Y_Trajectories(experimentIndex, :) = Eph(:, 3)'./10^3;
    MC_5_HPOP_ECI_Z_Trajectories(experimentIndex, :) = Eph(:, 4)'./10^3;
...
absMeanDeviationEndPosHPOP_5 = zeros( MCsampleNum, 1 );
absDeviationEndPosHPOP_5 = zeros( MCsampleNum, 1 );
relEndPosHPOP_5_chaser = MC_5_HPOP_PosEnd;
...
for dataIndex = 1 : MCsampleNum
    ...
    thisMeanEndPosHPOP_5 = MC_5_HPOP_PosEndMean( dataIndex, : );
    thisEndPosHPOP_5 = MC_5_HPOP_PosEnd( dataIndex, : );
    ...
    absMeanDeviationEndPosHPOP_5( dataIndex ) = norm( rECIManouverEnd_target - thisMeanEndPosHPOP_5' );
    ...
    relEndPosHPOP_5_chaser( dataIndex, : ) = relEndPosHPOP_5_chaser( dataIndex, : ) - rECIManouverEnd_target';
    ...
    absDeviationEndPosHPOP_5( dataIndex ) = norm( rECIManouverEnd_target - thisEndPosHPOP_5' );
...
for plotIndex = 1 : MCsampleNum
    ...
    plot3( MC_5_HPOP_ECI_X_Trajectories(plotIndex, :), MC_5_HPOP_ECI_Y_Trajectories(plotIndex, :), MC_5_HPOP_ECI_Z_Trajectories(plotIndex, :), 'm')
end
...
text( rECIExperimentEndSimple_chaser(1), rECIExperimentEndSimple_chaser(2), rECIExperimentEndSimple_chaser(3), 'Chaser Ideal Maneuver End' )
...
hold on
grid on
title('HPOP End Pos Error Difference with Acceleration Duration 2s vs. 1s, Delay Uniform +/- 2s')
plot( abs(relEndPosMeasurement_12))
plot( abs(relEndPosMeasurement_45))
