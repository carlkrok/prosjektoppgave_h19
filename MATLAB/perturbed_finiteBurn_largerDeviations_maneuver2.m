...
for experimentIndex = 1 : MCsampleNum
    ...
    combinedThrustEph = [beforeThrustEph(1:N_Step_BeforeThrust, :); precisionThrustEph(1:N_Step_PrecisionThrust, :); afterThrustEph(1:N_Step_AfterThrust, :)];
    %[beforeThrustEph(1:thrustPrecisionFactor:floor(N_Step_BeforeThrust/thrustPrecisionFactor)*thrustPrecisionFactor, :);...
    %    precisionThrustEph(1:thrustPrecisionFactor:floor(N_Step_PrecisionThrust/thrustPrecisionFactor)*thrustPrecisionFactor, :);...
    %    afterThrustEph(1:thrustPrecisionFactor:ceil(N_Step_AfterThrust/thrustPrecisionFactor)*thrustPrecisionFactor, :)]
    
    shortenedThrustEph = combinedThrustEph(1:thrustPrecisionFactor:end, :);
    
    Eph = [initialEph(1:N_Step_Initial, :); shortenedThrustEph; finalEph]; %
